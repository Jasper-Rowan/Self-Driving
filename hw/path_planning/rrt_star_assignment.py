...
def find_path(start, target, edges, radius=CAR_BUFFER, point_type='map'):
    ...
    while not end or counter < STEPS:
        ...
        minNode = nodes[0]
        dist = nodes[0].distance(random_point)
        for node in nodes:
            if node.distance(random_point) < dist:
                minNode = node
                dist = node.distance(random_point)
        ...
        for node in X_n:
            newCost = node.cost + node.distance(random_point)
        ...
        for node in X_n:
            newCost = random_point.cost + random_point.distance(node)
